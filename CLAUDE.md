...
- フォント: Archivo Black（見出し＝極太ブロック体）/ Inter・Noto Sans JP（本文）/ JetBrains Mono（数値）
- ヒーロー: cream 地（トークン連動でダークは反転）の**平面ポスター** — 三原色の円/三角/帯を組み、
  `--hero-p`（スクロール進捗）で緩慢に回転/ドリフト（kinetic）。3D プロウンは撤去（`ProunCanvas` 不使用）。
